...
import {ADD_TODO, REMOVE_TODO, UPDATE_TODO, TOGGLE,ADD_USER} from '../action-types';
...
export function todoReducer(store = todoDefaultStore, action) {
  switch (action.type) {
    ...
    case UPDATE_TODO: {
      ...
      // todo 1: (maybe done) добавить обработку toggle статуса тудушки
...
// todo 2: (maybe done) создать еще 1 редьюсер usersReducer
...
export const createRootReducer = () => {
  return combineReducers({
    counter,
    todoReducer,
    // todo 2: (maybe done) добавить тут usersReducer
    usersReducer
    // todo: (maybe done) обратите внимание, тут можно добавлять еще редьюсеры
  });
};