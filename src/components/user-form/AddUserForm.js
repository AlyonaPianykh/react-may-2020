...
class AddUserForm extends Component {
  ...
  onSubmit = (e) => {
    ...
    if (this.firstNameRef.current.value === '' || this.lastNameRef.current.value === '' || this.emailNameRef.current.value === ''|| this.addressRef.current.value === '') return;

    console.log(this.firstNameRef.current.value);
    console.log(this.lastNameRef.current.value);
    console.log(this.emailNameRef.current.value);
    ...
    // doentodo 3: зачистить форму, вызвав функцию onReset
    this.onReset();
  };

  // doentodo 3: добавить функцию onReset, которая будет зачищать поля формы
  ...
  render() {
    return (
      ...
      */}
      <button type="submit" onClick={this.onReset}>Reset my form data</button>
      </form>
    );
  }
}
