    ...
    render() {
        ...
        */}
                ...
                <div className="dropdown-toggle"
                     onClick={this.toggle}>{selectedOption/* тут будет выбранная опция*/}</div>
          ...
          */}
                        {
                            options.map(val => <div
                                className={`may-drop-down-options-wrapper-option dropdown-item ${selectedOption === val ? 'active' : ''}`}
                                key={val}
                                data-value={val} onClick={this.onOptionSelect}>{val}</div>)
                        }
                    </div>}
