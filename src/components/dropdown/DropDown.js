...
//  на базе нее будут рендериться или нет опции дропдауна
    ...
    //Кнопка on/off нашого списочку дропдавна
    toggle = () => {
...
//Наша кнопочка onclick на одну із наших div в яких є одне із значень 'Sort By Default', 'Sort By Author'
          ...
          */}
                      {
                        option.map(option=>{
                          ...
                          return(
                              <div className = "may-drop-down-options-wrapper-option dropdown-item" key={option} data-value={option} onClick={this.onOptionSelect}>
                                {option}
                              </div>
                          )
                        })
