...
//  на базе нее будут рендериться или нет опции дропдауна
  ...
  toggle = () => {
    // здесь должно быть открывание/закрывание дропдауна с помощью this.setState
  ...
  render() {
    ...
    return (
      <div className="may-drop-down dropdown">
        {/*  показать в строке 45 выбранную опцию selectedOption  и передать в onClick this.toggle
          ...
          */}
            {
              options.map(option => {
                return (
                    <div className = {`may-drop-down-options-wrapper-option dropdown-item ${option === selectedOption ? 'active' : ''}`} key={option} data-value={option} onClick={this.onOptionSelect}>{option}</div>
                )
              })
            }
          </div>)}
