    ...
    render() {
        ...
        return (
            <div className={`may-post-card card ${className}`}>
                ...
                {
                    showComments
                    ? <label className="btn btn-link" onClick={this.onToggleComments}>Hide comments</label>
                ...
                {
                    (showComments && !isCommentsLoading && isCommentsLoading && !comments.length) && <div>No comments for this post yet</div>
                }
                {
                    // todo если секция комментариев открыта, т.е. showComments = true
