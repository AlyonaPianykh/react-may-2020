  ...
  render() {
    ...
    return (
      <div className={`may-post-card card ${className}`}>
        ...
        <label onClick={this.onToggleComments} className='btn btn-link'>
        ...
        {
          (showComments && isCommentsLoading) && <LoadingPanel/>
        ...
        }
        {
          ...
          (showComments && !isCommentsLoading && commentsLoaded && !!comments.length) &&  comments.map(comment => (<Comment
              comment={comment}
              key={comment.id} />
              )
          )
