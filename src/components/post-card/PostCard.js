...
export class PostCard extends PureComponent {
    ...
    onToggleComments = () => {
        //  donetodo меняем в стейт значение showComments на противоположное (по аналогии как мы делали isOpen для PanelFromLecture
            ...
            <div className={`may-post-card card ${className}`}>
                ...
                */}
                {<label className="btn btn-link"
                        onClick={this.onToggleComments}>{showComments ? 'Hide comments' : 'Show comments'}</label>}

                {/* donetodo создать div который будет как children содержать error, если !!error */}
                {!!error && <div children={error}></div>}
               ...
               */}
                ...
                */}
                ...
                {/*donetodo если секция комментариев открыта, т.е. showComments = true
                   ...
                   то:
                */}
                {showComments && !isCommentsLoading && commentsLoaded && commentsLoaded && !!this.state.comments.length &&
