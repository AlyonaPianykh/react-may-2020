...
// todo 3: (done)сделать импорт Link из react-router-dom
...
class PostCard extends PureComponent {
  ...
  render() {
    //todo 3 : (done)достать ниже url из  props.match по аналогии с UserCard строка 7
    ...
    return (
      <div className={`may-post-card card ${className}`}>
        ...
        {/* todo 3 :(done)добавить ссылку Link на урлу с айди поста, где будут детали поста
                    по аналогии с 24 строкой в UserCard
        */}
        <Link to={`${url}/${post.id}`}>Show deteils</Link>
...
// todo 3: (done)подвязать PostCard с помощью withRouter к роутеру по аналогии с UserCard
