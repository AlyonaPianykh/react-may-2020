...
// done_todo: достать в строке 7 массив allComments из констант
import { postsList, allComments, usersList } from '../../constants'; // done_todo помимо константы postsList достать еще usersList
// done_todo: тут сделать импорт  UserCard из components/user-card/UserCard
...
function App() {
  ...
  return (
    <div className="App">
      ...
      <div className="d-flex posts-container">
        {
          postsList.map((item, index) => {
            const odd = index % 2 !== 0;
            // tado_todo: найти в массиве usersList пользователя, айди которого равно user_id в посте (т.е. в данном случае item)
            ...
            // done_todo: найти в массиве allComments комментарии, post_id которых = id поста  (т.е. в данном случае item.id)
      ...
      <div className="d-flex posts-container">
      {/*  done_todo: срендерить тут список пользователей, используя компонент UserCard */}
          {usersList.map(item =>{
              return(
