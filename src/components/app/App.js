      ...
      <div className="d-flex posts-container">
        {
          postsList.map((item, index) => {
            const odd = index % 2 !== 0;
              ...
              let author = usersList.find(value => {
                  return value.id === item.user_id
              });
              ...
              let comments = allComments.filter(value => value.post_id === item.id);
              return <Card post={item} key={item.id} hasImage={odd} author={`${author.first_name} ${author.last_name}`} comments={comments} />
          })
        }
      </div>
