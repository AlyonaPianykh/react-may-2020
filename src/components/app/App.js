            ...
            <div className="d-flex posts-container">
                {
                    postsList.map((item, index) => {
                        ...
                        //  пример тут: https://hackerthemes.com/bootstrap-cheatsheet/#blockquote-footer
                         const userIDmas = usersList.find((user)=>{
                        ...
                        const arrayFilterAllComments = allComments.filter((coment, id)=>{
                            return coment.post_id === item.id
                        });



                        return <Card post={item} key={item.id} author ={`${userIDmas.first_name} ${userIDmas.last_name}`} comments={arrayFilterAllComments} hasImage={odd}/>
                    })
                }
            </div>
