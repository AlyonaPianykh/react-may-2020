...
import {postsList, allComments, usersList} from '../../constants'; // donetodo помимо константы postsList достать еще usersList
    ...
    return (
        <div className="App">
            ...
            <div className="d-flex posts-container">
                {
                    postsList.map((item, index) => {
                        ...
                        let i = usersList.findIndex(el => el.id === item.user_id)
                        ...
                        let comments = allComments.filter(el => el.post_id === item.id)


                        return <Card comments={comments} author={`${usersList[i].first_name} ${usersList[i].last_name}`}
                                     post={item} key={item.id} hasImage={odd}/>
                    })
                }
            </div>
            ...
            <div className="d-flex posts-container">
                ...
                {usersList.map(val => <UserCard user={val} key={val.id}/>)}
