...
function App() {
    console.log(["0000",postsList, allComments, usersList]);
    const renderList = () => {
    const res = [];
      ...
      <div className="d-flex posts-container">
        {
          postsList.map((item, index) => {
            ...
            const postComments = allComments.filter(comment => comment.post_id === item.id);
              console.log(['sdfdgdfhfl',postComments]);
              // todo: найти в массиве usersList пользоват[еля, айди которого равно user_id в посте (т.е. в данном случае item)
