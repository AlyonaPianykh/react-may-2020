...
class PostsList extends Component {
  state = {
    posts: [],
    isLoading: false
    // +odo 2: добавить isLoading индикатор
  };

  componentDidMount() {
    // +odo 2: вызвать загрузку постов
    this.loadPosts();
  ...
  render() {
    // +odo 2: достать также лоадинг индикатор из стейта
    const { posts, isLoading } = this.state;

    return (
      <div className='d-flex justify-content-center flex-wrap'>

        {/* +odo 2: ниже добавить проверку если сейчас идет загрука то показываем лоадинг индикатор (как в задании 7)
