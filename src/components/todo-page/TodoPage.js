...
// DONEtodo 1: импортнуть в этом файле функцию на toggle статуса тудушки
//         подумайте какие еще шаги нужно выполнить, чтоб все заработало
import {addTodo, removeTodo, updateTodo, setStatusTodo} from '../../actions';
...
class TodoPage extends Component {
    ...
    setStatusTodo= (todo) =>{
        // debugger
        const {setStatusTodo} = this.props;


        return () => {
            todo.doneStatus = !todo.doneStatus;
            //debugger
            setStatusTodo && setStatusTodo(todo);
        }
    }
            ...
            <div>
                ...
                <div className="d-flex flex-column m-2">
                    ...
                    <div className="d-flex">
                        {!isEditMode &&
                        <button className="btn btn-primary m-2" onClick={this.addTodo}>Add todo</button>}
                        {isEditMode &&
                        <button className="btn btn-primary m-2" onClick={this.updateTodo}>Update todo</button>}
                    </div>
                </div>
                  ...
                  */}
                                    <label>
                                        ...
                                        <input type="checkbox"
                                               checked={doneStatus}
                                               className="m-1"
                                               onChange={this.setStatusTodo(todo)}
                                        />
                                    </label>
